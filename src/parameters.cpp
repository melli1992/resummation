...
double mt(173.1);//mt(173.0);
double mt2(pow(mt,2));
double mb(4.18);// should it be in MSbar?
double mb2(pow(mb,2));
double mZ(91.1876);
double mZ2(pow(mZ,2));
double mW(80.419002);
double mW2(pow(80.419002,2));
...
double ISNNLL(1);
double ISNLP(1);
bool setdym = false, SCET=false, BN = false, DY=true, higgs=false, hh=false, WW=false, ZZ=false, full=true, diff=false, LO=true, NLO=true, NNLO=true, RES=true, realPDF=false, fitPDF=true, SUSY = false;
...
void update_defaults(bool printout , bool pdfset){
	...
	if(printout){
		...
		else if(NLO) cout << "computing up to NLO" << endl;
		else if(LO) cout << "computing up to LO" << endl;
		if(SCET) cout << "Resumming in SCET, order: ";
		else cout << "Resumming in pQCD, order: ";
		if(RES) cout << "Resumming LL: "<< (bool) ISLL << ", NLL: "<< (bool) ISNLL << ", NNLL: "<< (bool) ISNNLL << ", NLP: "<< (bool) ISNLP << endl;
		if(fitPDF){cout << "Using the fitted PDFs" << endl; realPDF = false;}
		if(realPDF) cout << "Using the real PDFs" << endl;
		cout << "=========================================" << endl;
	}
}
